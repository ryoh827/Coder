namespace Coder {
  ...
  async function solve() {
    ...
    const [_, M]: number[] = (await sc.getLine()).split(' ').map(Number);
    const scores: number[] = (await sc.getLine()).split(' ').map(Number);

    let ans = 0;

    for (const i of (await sc.getLine()).split(' ').map(Number)) {
      ans += scores[i - 1];
    }
